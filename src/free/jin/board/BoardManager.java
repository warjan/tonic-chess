...
public class BoardManager extends Plugin implements GameListener, UserMoveListener,
  ...
  public void connectionLost(Connection conn){
    ...
    for (int i = 0; i < games.size(); i++){
      gameEndCleanup((Game)games.get(i));
    }
    /*JDialog connLostJDialog = new JDialog(); 
    DialogPanel connLost = new DialogPanel((Frame)containers.elementAt(0), "Warning!");
    JLabel connLostLabel = new JLabel("Connection with server lost.");
    connLost.add(connLostLabel);
    
    connLost.setVisible(true);*/
