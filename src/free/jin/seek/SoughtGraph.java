...
public class SoughtGraph extends JComponent{
  ...
  protected static final int BULLET_SLOTS = 10;
  ...
  protected static final int BLITZ_SLOTS = 32;
  ...
  protected static final int STANDARD_SLOTS = 18;
  ...
  protected static final double GRAPH_HEIGHT_PERCENTAGE = 9.3/10.0;
  ...
  private final Image bgImage;
     ...
     * The user's rating.
     */
   //TODO Make tonic aware of user's rating for diffrent types of games. Implement methods to handle in connection classes, event and listeners. Uf!
    private int rating;
  ...
  public SoughtGraph(Plugin plugin){
    this.plugin = plugin;
      ...
      //this.rating = Jin.getInstance().getConnManager().getSession().getUser().getUsername()
  ...
  public void paintComponent(Graphics g){
    ...
    // The axises' arrows
    g2d.fillPolygon(new int[]{graphX+5,graphX-5,graphX},new int[]{graphY+9,graphY+9,graphY},3);
    g2d.fillPolygon(new int[]{graphX+graphWidth,graphX+graphWidth-9,graphX+graphWidth-9},new int[]{graphY+graphHeight,graphY+graphHeight-5,graphY+graphHeight+5},3);


    g2d.setColor(lightFG);
    
    // The line separating bullet from blitz.
    int bbX = (int)(graphX+BULLET_SLOTS*slotWidth);
    g2d.drawLine(bbX, graphY, bbX, graphY+graphHeight);

    // The line separating blitz from standard.
    int bsX = (int)(graphX+(BULLET_SLOTS+BLITZ_SLOTS)*slotWidth);
    g2d.drawLine(bsX, graphY, bsX, graphY+graphHeight);
    ...
    g2d.setFont(timeStringFont);
    int timeStringY = graphY+graphHeight+timeStringFontMetrics.getMaxAscent()+1;
    g2d.drawString(bulletString, graphX+(bulletWidth-bulletStringWidth)/2, timeStringY);
    g2d.drawString(blitzString, graphX+bulletWidth+(blitzWidth-blitzStringWidth)/2, timeStringY);
    g2d.drawString(standardString, graphX+bulletWidth+blitzWidth+(standardWidth-standardStringWidth)/2, timeStringY);

    // The "1000", "1500" and "2000" strings.
    String tenString = "1000";
    String fifteenString = "1500";
    String twentyString = "2000";
    int ratingStringHeight = graphHeight/6; // This is obviously wrong, but we know for sure the height isn't going to limit us.
    ...
    g2d.setFont(ratingStringFont);
    int ratingStringX = 1;
    g2d.drawString(tenString, ratingStringX, (int)(graphY+graphHeight-6*slotHeight));
    g2d.drawString(fifteenString, ratingStringX, (int)(graphY+graphHeight-16*slotHeight));
    g2d.drawString(twentyString, ratingStringX, (int)(graphY+graphHeight-26*slotHeight));
    ...
    if (curSeek != null){
      ...
      g.drawString(seekString, 1, height-seekStringHeight+seekFontMetrics.getMaxAscent());*/
        this.setToolTipText(seekString);
    }
    if (curSeek == null){
