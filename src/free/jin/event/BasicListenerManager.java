...
public class BasicListenerManager implements ListenerManager, SeekListenerManager,
  ...
  public void fireFriendsEvent(FriendsEvent evt){
    ...
    public void removeChannelsListener(ChannelsListener listener) {
        if (!(source instanceof ChannelsConnection)){
            throw new free.util.UnsupportedOperationException("The source connection is not an instance of ChannelsConnection");
        }

        listenerList.remove(ChannelsListener.class, listener);
    }

    public void addChannelsListener(ChannelsListener listener) {
          if (!(source instanceof ChannelsConnection)){
            throw new free.util.UnsupportedOperationException("The source connection is not an instance of ChannelsConnection");
        }

        listenerList.add(ChannelsListener.class, listener);
    }

    public void fireChannelsEvent(ChannelsEvent evt){
          if (!(source instanceof ChannelsConnection)){
            throw new free.util.UnsupportedOperationException("The source connection is not an instance of ChannelsConnection");
          }
          Object[] listenerList = this.listenerList.getListenerList();
          for (int i = 0; i < listenerList.length; i += 2){
            if (listenerList[i] == ChannelsListener.class){
                ChannelsListener listener = (ChannelsListener)listenerList[i+1];
                try{
                    switch (evt.getId()){
                        case ChannelsEvent.CHANNEL_ADDED:
                            listener.channelAdded(evt);
                            break;
                        case ChannelsEvent.CHANNEL_REMOVED:
                            listener.channelRemoved(evt);
                            break;
                        case ChannelsEvent.USER_CHANNEL_LIST_RECEIVED:
                            listener.channelListReceived(evt);
                            break;
                    }
                }
                catch (RuntimeException e){
    ...
    public void fireMessageEvent(MessageEvent evt){
        Object[] listenerList = this.listenerList.getListenerList();
        for (int i = 0; i < listenerList.length; i += 2){
