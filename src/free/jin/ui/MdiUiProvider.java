...
public class MdiUiProvider extends AbstractUiProvider {
    ...
    public void showDialog(DialogPanel dialog, Component parent) {
        ...
        dialog.show(new JDialog(parentFrame),
            (parent == null) ? parentFrame : parent);
    }
    ...
    public PluginUIContainer createPluginUIContainer(Plugin plugin, String id,
        int mode) {
        AbstractPluginUIContainer container = new InternalFramePluginUIContainer(plugin,
                id, mode);
    ...
    private class ConnectionMenu extends JMenu implements ActionListener,
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (source == newConnection) {
                ...
                User user = (User) recentAccounts.elementAt(index -
                        separatorIndex - 1);
        ...
        private void updateRecentAccountsMenuItems() {
            ...
            for (int i = 1; i <= recentAccounts.size(); i++) {
                ...
                String label = i + " " + user.getUsername() + " at " +
                    user.getServer().getShortName();
        ...
        private Vector loadRecentAccounts() {
            ...
            for (int i = 0; i < count; i++) {
                String username = prefs.getString("accounts.recent." + i +
                        ".username");
                String serverId = prefs.getString("accounts.recent." + i +
                        ".serverId");
    ...
    private class MdiPrefsMenu extends PrefsMenu {
        ...
        public MdiPrefsMenu() {
            ...
            bgMenuItem.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent evt) {
                        Preferences prefs = Jin.getInstance().getPrefs();
                        String wallpaperFilename = prefs.getString("desktop.wallpaper.filename",
                                null);
                        File currentImageFile = (wallpaperFilename == null)
                            ? null : new File(wallpaperFilename);

                        BackgroundChooser bChooser = new BackgroundChooser(mainFrame,
                                desktop, null, null,
                                AdvancedJDesktopPane.CENTER,
                                desktop.getBackground(), currentImageFile,
                        ...
                        prefs.setString("desktop.wallpaper.filename",
                            (chosenImageFile == null) ? null
                                                      : chosenImageFile.getAbsolutePath());
    ...
    private class InternalFramePluginUIContainer
        extends AbstractPluginUIContainer implements VetoableChangeListener,
        ...
        public boolean isVisible() {
            ...
            ((frame.getDesktopIcon() != null) &&
            (frame.getDesktopIcon().getParent() != null));
        }
        ...
        private void show() {
            ...
            firePluginUIEvent(new PluginUIEvent(this,
                    PluginUIEvent.PLUGIN_UI_SHOWN));
        }
        ...
        private void hide() {
            ...
            // end workaround
            firePluginUIEvent(new PluginUIEvent(this,
                    PluginUIEvent.PLUGIN_UI_HIDDEN));
        }
        ...
        public void vetoableChange(PropertyChangeEvent pce)
            ...
            if (pce.getPropertyName().equals(JInternalFrame.IS_CLOSED_PROPERTY) &&
                ...
                case SELF_MANAGED_CONTAINER_MODE:
                    firePluginUIEvent(new PluginUIEvent(this,
                            PluginUIEvent.PLUGIN_UI_CLOSING));
        ...
        protected void saveState() {
            ...
            if (frame.isMaximum() != prefs.getBool(prefix +
                        "iframe.isMaximized", false)) {
                prefs.setBool(prefix + "iframe.isMaximized", frame.isMaximum());
            }

            if (frame.isIcon() != prefs.getBool(prefix + "iframe.isIconified",
                        false)) {
            ...
            bounds.scale(1 / (double) desktop.getWidth(),
                1 / (double) desktop.getHeight());
        ...
        protected void loadState() {
            ...
            RectDouble defaultBounds = getInitialBounds(frame,
                    mainFrame.getContentPane().getSize());
            RectDouble bounds = ((id == null) ? defaultBounds
                                              : prefs.getRectDouble(prefix +
                    "iframe.bounds", defaultBounds));
            ...
            bounds.setWidth(Math.min(desktopWidth,
                    Math.max(40, bounds.getWidth())));
            bounds.setHeight(Math.min(desktopHeight,
                    Math.max(40, bounds.getHeight())));

            frame.setBounds(bounds.toRect());

            try {
                frame.setMaximum(prefs.getBool(prefix + "iframe.isMaximized",
                        false));
        ...
        public void internalFrameActivated(InternalFrameEvent e) {
            firePluginUIEvent(new PluginUIEvent(this,
                    PluginUIEvent.PLUGIN_UI_ACTIVATED));
        }

        public void internalFrameDeactivated(InternalFrameEvent e) {
            firePluginUIEvent(new PluginUIEvent(this,
                    PluginUIEvent.PLUGIN_UI_DEACTIVATED));
        }
        ...
        private class MdiInternalFrame extends FixedJInternalFrame {
            ...
            public Dimension getMinimumSize() {
                ...
                Dimension dim = this.getContentPane().getLayout()
                                    .minimumLayoutSize(this.getContentPane());
    ...
    private class FocusManager extends DefaultFocusManager {
        ...
        public void processKeyEvent(Component focusedComponent, KeyEvent evt) {
            if (!SwingUtilities.isDescendingFrom(evt.getComponent(),
            ...
            if (keyCode == KeyEvent.VK_F8 ){
                frameSwitcher.selectMainConsole();
            }
        }
    }
    ...
    private class DesktopManager extends DefaultDesktopManager {
        ...
        public void resizeFrame(JComponent f, int newX, int newY, int newWidth,
            ...
            boolean right = ((newX + newWidth) != (jifBounds.x +
                jifBounds.width));
            boolean bottom = ((newY + newHeight) != (jifBounds.y +
                jifBounds.height));
            ...
            int xMargin = (jifBounds.width < X_MARGIN) ? jifBounds.width
                                                       : X_MARGIN;
            int yMargin = (jifBounds.height < Y_MARGIN) ? jifBounds.height
                                                        : Y_MARGIN;
            ...
            if (left) {
                x1 = Math.min(Math.max(x1, x2 - desktopSize.width),
                        desktopSize.width - xMargin);
            }

            if (top) {
                y1 = Math.max(Math.min(Math.max(y1, 0),
                            desktopSize.height - yMargin),
        ...
        public void dragFrame(JComponent f, int newX, int newY) {
            ...
            newX = Math.max(Math.min(newX, desktopSize.width - xMargin),
                    xMargin - f.getWidth());
