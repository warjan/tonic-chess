...
public abstract class VectorPiecePainter extends AbstractColoredPiecePainter{
  ...
  private Polygon kingPolygon, queenPolygon, rookPolygon, bishopPolygon, knightPolygon, pawnPolygon;
    ...
    /**
     * The pieces stroke width.
     */
    
    private BasicStroke pieceStroke = new BasicStroke(2.0f);
  ...
  public final void paintPiece(Piece piece, Graphics g, Component component, Rectangle rect,
    ...
    if (cPiece.isKing())
      drawKingImage(g2, width, height, pieceColor, outlineColor);
    else if (cPiece.isQueen())
      drawQueenImage(g2, width, height, pieceColor, outlineColor);
    else if (cPiece.isRook())
      drawRookImage(g2, width, height, pieceColor, outlineColor);
    else if (cPiece.isBishop())
      drawBishopImage(g2, width, height, pieceColor, outlineColor);
    else if (cPiece.isKnight())
      drawKnightImage(g2, width, height, pieceColor, outlineColor);
    else if (cPiece.isPawn())
      drawPawnImage(g2, width, height, pieceColor, outlineColor);
  ...
  protected void drawKingImage(Graphics g, int width, int height,
    ...
    g2.fillPolygon(kingPolygon);
    
      g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
    g2.setColor(outlineColor);
    g2.drawPolygon(kingPolygon);
  }
  ...
  protected void drawPawnImage(Graphics g, int width, int height,
    ...
    g2.setColor(pieceColor);
    g2.fillPolygon(pawnPolygon);

    g2.setColor(outlineColor);
    g2.drawPolygon(pawnPolygon);
  }    
