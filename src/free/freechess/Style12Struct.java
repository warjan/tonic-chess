...
public class Style12Struct extends Struct{
  ...
  public Style12Struct(final String boardLexigraphic, final String currentPlayer,
	  final int doublePawnPushFile, final boolean canWhiteCastleKingside, 
	  final boolean canWhiteCastleQueenside, final boolean canBlackCastleKingside,
	  final boolean canBlackCastleQueenside, final int pliesSinceIrreversible,
	  final int gameNumber, final String whiteName, final String blackName, final int gameType,
	  final boolean isPlayedGame, final boolean isMyTurn, final int initTime, final int increment,
	  final int whiteMaterialStrength, final int blackMaterialStrength, final int whiteTime,
	  final int blackTime, final int nextMoveNumber, final String moveVerbose, final String moveSAN,
	  final int moveTime, final boolean isBoardFlipped, final boolean isClockRunning, final int lag){
  ...
  public static Style12Struct parseStyle12Line(String line){
    ...
    final StringBuffer posBuf = new StringBuffer(64);
    for (int i = 0; i < 8; i++)
      posBuf.append(tokens.nextToken()); // The board
    final String positionLexigraphic = posBuf.toString();

    final String currentPlayer = tokens.nextToken(); // The color string, either "B" or "W"

    final int doublePawnPushFile = Integer.parseInt(tokens.nextToken()); // The double pawn push file

    final boolean canWhiteCastleKingside = parseBoolean(tokens.nextToken()); // Can white castle kingside
    final boolean canWhiteCastleQueenside = parseBoolean(tokens.nextToken()); // Can white castle queenside
    final boolean canBlackCastleKingside = parseBoolean(tokens.nextToken()); // Can black castle kingside
    final boolean canBlackCastleQueenside = parseBoolean(tokens.nextToken()); // Can black castle queenside

    final int pliesSinceIrreversible = Integer.parseInt(tokens.nextToken()); // Number of plies since an irreversible move

    final int gameNumber = Integer.parseInt(tokens.nextToken()); // The game number

    final String whiteName = tokens.nextToken(); // White's name
    final String blackName = tokens.nextToken(); // Black's name

    final int myRelation = Integer.parseInt(tokens.nextToken()); // My relation to the game
    int gameType;
    ...
    final boolean isMyTurn = myRelation > 0;

    final int initTime = 60*Integer.parseInt(tokens.nextToken()); // Initial time
    final int increment = Integer.parseInt(tokens.nextToken()); // Increment

    final int whiteMaterialStrength = Integer.parseInt(tokens.nextToken()); // White's material strength
    final int blackMaterialStrength = Integer.parseInt(tokens.nextToken()); // Black's material strength

    final int whiteTime = Integer.parseInt(tokens.nextToken()); // White's remaining time
    final int blackTime = Integer.parseInt(tokens.nextToken()); // Black's remaining time

    final int nextMoveNumber = Integer.parseInt(tokens.nextToken()); // The number of the next move
    ...
    final StringTokenizer timeTokens = new StringTokenizer(moveTimeString, ":.");
    final int minutes = Integer.parseInt(timeTokens.nextToken());
    final int seconds = Integer.parseInt(timeTokens.nextToken());
    final int milliseconds = Integer.parseInt(timeTokens.nextToken());
    final int moveTime = 60*1000*minutes + 1000*seconds + milliseconds;

    String moveSAN = tokens.nextToken(); // The move in SAN notation
    if (moveSAN.equals("none"))
      moveSAN = null;

    final boolean isBoardFlipped = parseBoolean(tokens.nextToken()); // Is the board flipped?

    final boolean isClockRunning = parseBoolean(tokens.nextToken()); // Is the clock of the player to move running?

    final int lag = Integer.parseInt(tokens.nextToken()); // The lag, in milliseconds.
  ...
  public String getBoardFEN(){
    ...
    final String boardLexigraphic = getBoardLexigraphic();
    int emptySquareCounter = 0;
    for (int i = 0; i < 8; i++){
      for (int j = 0; j < 8; j++){
        final char c = boardLexigraphic.charAt(j+i*8);
        if (c == '-')
          emptySquareCounter++;
        else{
    ...
    final StringBuffer castlingAvailability = new StringBuffer();
  ...
  /**
   * Returns a string representing current user's name.
   */
  
  public String getMovingPlayerName(){
      String s;
      if (this.getCurrentPlayer().equals("W")){
          s = this.getBlackName();
      }
      else {
          s = this.getWhiteName();
