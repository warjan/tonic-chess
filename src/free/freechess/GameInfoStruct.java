...
public class GameInfoStruct extends Struct{
  ...
  public GameInfoStruct(final int gameNumber, final boolean isGamePrivate, final String gameCategory,
      final boolean isGameRated, final boolean isWhiteRegistered, final boolean isBlackRegistered,
      final int whiteTime, final int whiteInc, final int blackTime, final int blackInc, final int partnerGameNumber,
      final int whiteRating, final char whiteProvShow, final int blackRating, final char blackProvShow, 
      final boolean isWhiteTimesealed, final boolean isBlackTimesealed){
  ...
  public static GameInfoStruct parseGameInfoLine(final String line){
    final StringTokenizer tokens = new StringTokenizer(line, " ,=");

    if (!tokens.nextToken().equals("<g1>")) // Skip the <g1> identifier
      throw new IllegalArgumentException("Missing \"<g1>\" identifier");

    final int gameNumber = Integer.parseInt(tokens.nextToken()); // Game number

    assertToken(tokens, "p"); // p=
    final boolean isGamePrivate = parseBoolean(tokens.nextToken()); // Is the game private?

    assertToken(tokens, "t"); // t=
    final String gameType = tokens.nextToken(); // Game type

    assertToken(tokens, "r"); // r=
    final boolean isGameRated = parseBoolean(tokens.nextToken()); // Is the game rated?

    assertToken(tokens, "u"); // u=
    final boolean isWhiteRegistered = parseBoolean(tokens.nextToken()); // Is white registered?
    final boolean isBlackRegistered = parseBoolean(tokens.nextToken()); // Is black registered?

    assertToken(tokens, "it"); // it=
    final int whiteTime = Integer.parseInt(tokens.nextToken()); // White's initial time
    final int whiteInc = Integer.parseInt(tokens.nextToken()); // White's increment    

    assertToken(tokens, "i");
    final int blackTime = Integer.parseInt(tokens.nextToken()); // Black's initial time
    final int blackInc = Integer.parseInt(tokens.nextToken()); // Black's increment

    assertToken(tokens, "pt");
    final int partnerGameNumber = Integer.parseInt(tokens.nextToken()); // The partner's game number
  ...
  private static void assertToken(final StringTokenizer tokenizer, final String token){
